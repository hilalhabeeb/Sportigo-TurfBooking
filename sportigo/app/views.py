...
def providerhome(request):
    if 'email' in request.session:
        ...
        # Retrieve club bookings for the provider's turfs
        clubbookings = ClubBooking.objects.filter(turf_listing__in=your_turfs)
...
def bookingsummary(request):
    if 'email' in request.session:
        ...
        # Retrieve regular bookings for the provider's turfs
        bookings = Booking.objects.filter(turf_listing__in=your_turfs)

        # Retrieve club bookings for the provider's turfs
        clubbookings = ClubBooking.objects.filter(turf_listing__in=your_turfs)

        # Retrieve club user information for each club booking
        for clubbooking in clubbookings:
            club_user = clubbooking.user
            clubbooking.mobile_number = club_user.contact_number
            clubbooking.club_id = club_user.club_id

        context = {
            'provider_name': provider.venue_name,
            'your_turfs': your_turfs,
            'bookings': bookings,
            'clubbookings': clubbookings
        }

        response = render(request, 'bookingsummary.html', context)
        response['Cache-Control'] = 'no-store, must-revalidate'
        return response
